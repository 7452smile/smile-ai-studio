...
````yaml get /v1/ai/image-to-prompt/{task-id}
openapi: 3.0.0
info:
  description: >-
...
security:
  - apiKey: []
paths:
  /v1/ai/image-to-prompt/{task-id}:
    get:
      tags:
        - image-to-prompt
      summary: Image to Prompt - Get task status
      description: Get the status of one image-to-prompt task
      operationId: get_image_to_prompt_task_status
      parameters:
        ...
        '200':
          content:
            application/json:
              schema:
                $ref: >-
                  #/components/schemas/get_image_to_prompt_task_status_200_response
...
components:
  schemas:
    get_image_to_prompt_task_status_200_response:
      example:
        data:
          generated:
